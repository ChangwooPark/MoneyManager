...
│  지출  ¥ 85,000              │
│  ─────────────────────────  │
│  예산 ¥100,000 중 ¥15,000 남음│
│  [████████████░░░░] 85%     │  ← 예산 진행 바
└─────────────────────────────┘
```

- 예산 미설정 시 진행 바 섹션 비표시
- 예산 초과 시 진행 바를 `var(--expense)` 색상으로 강조

**구현 방향**

- `HomeTab.tsx` 상단 요약 카드에 예산 정보 추가
- `getBudget(yearMonth)` 호출 — 기존 API 재사용
- 번역 키 추가: `homeBudgetRemain`, `homeBudgetOver`, `homeBudgetNone`
