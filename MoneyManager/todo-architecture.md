...
### Phase 14: 홈·달력 탭 UX 개선 + 더보기 탭 기능 보강
...
- [ ] **아코디언 단일 열림 — 다른 항목 클릭 시 이전 항목 자동 닫힘**
...
- [ ] **거래 입력 모달 — 드래그 인식 영역 확대 및 배경 스크롤 차단**
  - 현재 문제: 모달이 열린 상태에서 드래그(닫기 시도)를 하면, 모달보다 뒤의 홈/통계 화면이 먼저 스크롤됨
  - 개선 1: 모달 핸들 바(드래그 영역)를 현재보다 더 넓게 확장
  - 개선 2: 모달이 열려있는 동안 배경(`body` 또는 메인 스크롤 영역)에 `overflow: hidden` 적용하여 배경 스크롤 차단
  - 모달의 터치/드래그 이벤트가 배경으로 전파되지 않도록 이벤트 전파 차단(`e.stopPropagation()`) 검토
  - 홈 탭과 동일하게 통계 탭에서도 적용 필요
...
- [ ] **카테고리 항목 클릭 시 상세 내역 표시**
  ...
  - 상세 내역 구성:
    ...
    - 거래 목록: 날짜 / 금액 / 메모 (홈 탭 거래 항목과 유사한 형태)
    - 닫기: ✕ 버튼 또는 오버레이 클릭 / 드래그 다운으로 닫기
  - 구현 방식 후보:
