...
- [ ] PIN 입력 화면 구현 (앱 최초 진입 시 노출)
  - 초기 마스터 PIN: `8907`
  - Firestore 시스템 설정 도큐먼트와 비교 인증
  - 숫자 패드(NumPad) UI 구현
- [ ] 인증 성공 시 세션 유지 처리
- [ ] PIN 변경 기능 ('더보기' 탭 내 구현)
  - 현재 PIN 확인 후 새 PIN으로 업데이트

### Phase 8: 공통 레이아웃 및 네비게이션
- [ ] 모바일 퍼스트 반응형 레이아웃
  - 모바일: 전체 화면 앱 스타일
  - PC/태블릿: `max-w-md` 중앙 배치 (앱 뷰 유지)
  - 모든 터치 타깃 최소 44×44px 확보
- [ ] 하단 고정 네비게이션 바 (탭바) 구현
  - 탭 1: 홈 (일일 내역)
  - 탭 2: 달력
  - 탭 3: 통계
  - 탭 4: 더보기
  - 활성 탭 표시, 모달 오픈 시에도 항상 최하단 고정
- [ ] 공통 연월 선택기 컴포넌트 (이전달 < 년월 > 다음달)
